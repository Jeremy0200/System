...
class AlumnosController extends Controller
{
    ...
    public function verificarCedula(Request $request, CedulaService $cedulaService)
    {
        $cedula = $request->input('cedula');
        $resultado = $cedulaService->verificarCedula($cedula);

        if ($resultado) {
            return response()->json(['message' => 'Cédula válida', 'data' => $resultado], 200);
        } else {
            return response()->json(['message' => 'Cédula no válida o no encontrada'], 400);
        }
    }
